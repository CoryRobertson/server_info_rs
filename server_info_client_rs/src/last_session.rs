pub mod last_session {
    ...
    impl LastSession {
        fn convert_to_string(&self) -> String {
            ...
            let _ = s.write_str(",");
            let _ = s.write_str(&*format!(
                "{},{}",
                self.screen_dimension.0.to_string(),
                self.screen_dimension.1.to_string()
            ));
    ...
    pub fn write_to_file(file_name: &str, last_session: LastSession) -> Result<(), String> {
        ...
        let mut file = match File::create(&path) {
            Ok(f) => f,
            Err(e) => {
                return Err(format!("Could not create file: {}, {}", e, display).to_string());
            }
        };
        ...
        match file.write_all(last_session.convert_to_string().as_bytes()) {
            Ok(_) => {}
            Err(e) => {
                return Err(format!("Could not write to file: {}, {}", e, display).to_string());
            }
        };
